    ...
    <>
      <div className="flex flex-col h-screen p-4 bg-slate-900 shadow-xl w-64 text-slate-100 border-r border-slate-700 hidden md:block">
        ...
        <div className="mt-auto border-t border-slate-700 pt-5 space-y-3">
          ...
          {/* ปุ่ม Logout */}
          <a className="flex items-center gap-3 w-full px-4 py-2.5 rounded-lg text-sm font-medium text-rose-400 hover:bg-rose-500/10 transition-colors cursor-pointer" onClick={() => signOut({ redirect: true, callbackUrl: "/login" })}>
      ...
      {isOpenMenu ? (
        <div className="flex flex-col h-screen p-4 bg-slate-900 shadow-xl w-64 text-slate-100 border-r border-slate-700 block md:hidden">
        {/* --- ส่วนหัว: Logo / Name --- */}
          <div className="flex items-center gap-3 px-3 py-4 mb-10 border-b border-slate-700">
            <div className="p-2.5 bg-green-500 rounded-xl shadow-inner">
              <Sprout className="w-8 h-8 text-white" strokeWidth={1.5}/>
